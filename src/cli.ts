...
const run = async (options?: { configFile?: string; port?: number }, isServe = false) => {
  ...
  if (!options?.configFile) {
    cwd = process.cwd();
    configTSFile = path.join(cwd, 'ylm.config.ts');
    configJSFile = path.join(cwd, 'ylm.config.js');
...
if (require.main === module) {
  const argv = yargs(process.argv)
    ...
    .alias('p', 'port')
    .alias('version', 'v')
    .example('$ npx ylm', '生成 mock 文件')
    .example('$ npx ylm -c=配置文件路径', '指定配置文件并生成 mock 文件')
    .example('$ npx ylm serve', '启动 mock 服务器，默认端口号为 3000')
    .example('$ npx ylm serve -p=端口号', '指定端口启动 mock 服务器')
    .example('$ npx ylm init', '初始化配置文件，默认配置文件类型为 ts')
    .example('$ npx ylm init -t=ts|js', '指定文件类型初始化配置文件').argv;
